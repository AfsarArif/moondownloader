...
*Run against a PRISTINE gen_1.py. Every anchor is an exact string from that
...
def method_span(src: str, name: str, indent: str = "    ",
    ...
    *`after` is not optional in practice: gen_1.py has three `def __init__` at
...
def main() -> int:
    ...
    source = root / "gen_1.py"
    if not source.exists():
        raise SystemExit(f"[FAIL] {source} not found")
    original = source.read_text(encoding="utf-8")
    if "class Engine:" in original:
        raise SystemExit("[FAIL] gen_1.py looks already transformed -- use a pristine copy")
